...
//        List<StateInfo> lastFiveStates = states.stream().collect(Collectors.toList());
//        System.out.println("The Size"+lastFiveStates.stream().count());// the size 50

        List<StateInfo> lastFiveStates = states.stream().skip(45).collect(toList());
//        for(StateInfo lF:lastFiveStates ) {
...
//        }
        ...
        StateInfo cardinalState = states.stream().filter(bird ->bird.getStateBird().equals("cardinal")).findAny().orElseThrow(() -> new RuntimeException("No state with cardinal as the state bird found"));
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(state ->state.getStateAnthem().equals("anthem"));
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream().noneMatch(state -> state.getStateMotto().split(" ").length == 1);
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream().mapToInt(state->state.getCapital().getAverageYearlyPrecipitationInInches()).sum();
        ...
        Map<String, Long> numberOfStateCapitalsByTimeZone = states.stream()
                .collect(Collectors.groupingBy(state -> state.getCapital().getTimeZone(),
                        Collectors.counting()));
...
//        String allStateNamesSemicolonDelimited = null;
        ...
        String allStateNamesSemicolonDelimited = states.stream()
        ...
        List<String> allDistinctStateBirds = states.stream()
                .map(state-> state.getStateBird())
                .distinct()
                .collect(Collectors.toList());
        testResults.put("A23", StatesAndCapitalsCheck.adv23(allDistinctStateBirds));

//         .map(StateInfo::getStateBird)
        ...
        Integer maxStateElevation = states.stream()
                .map(state->state.getHighestElevationInFeet())
